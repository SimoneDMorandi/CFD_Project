...
global ischeme % 1 -> FDS-Osher;  2 -> Osher O-ordering;  3 -> Osher P-ordering
...
for n=2:ncmm
    ...
    % Identical setup for all three schemes.
    pa  =  p(nm);
    pb  =  p(np);
    ua  =  u(nm);
    ub  =  u(np);
    ...
    elseif ischeme == 2
        ...
        % robustness guard for strong expansions
        if ac <= 0.0 || ad <= 0.0
